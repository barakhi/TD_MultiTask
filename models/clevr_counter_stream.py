...
class BUTD_comp_CLEVR_vgg(nn.Module):
    def __init__(self, num_tasks, block, num_blocks):
        ...
        #self.classifier2 = VGGclassifier(num_classes=8)
        #self.classifier3 = VGGclassifier(num_classes=8)
        #self.classifier4 = VGGclassifier(num_classes=8)
    ...
    def forward(self, image, task):
        ...
        #td_e4 = self.conv_emb(task_emb.view(bs, 32, 8, 8)) + self.lat1_4(out_l4)

        td_first = self.conv_emb(task_emb.view(bs, 32, 7, 7))
        #td_first = F.interpolate(td_first, scale_factor=2, mode='nearest')
        ...
        #td_d1 = td_e2 + self.lat1_0(out_)  # ==================> loss
        #td_c1 = td_d1  # F.interpolate(td_d1, scale_factor=2, mode='nearest')
        #td_e1 = self.TDlayer1(td_c1)  # ==================> loss
        #td_e1_l = self.lat2_1(td_e1)
        ...
        #out3 = self.classifier3(rep)
        #out4 = self.classifier4(rep)

        #cur_task = torch.cat((torch.sum(task[:, :10], dim=1).unsqueeze(1), torch.sum(task[:, 10:20], dim=1).unsqueeze(1),
        #        torch.sum(task[:, 20:30], dim=1).unsqueeze(1), torch.sum(task[:, 30:40], dim=1).unsqueeze(1)), dim=1).byte()


        #out = torch.masked_select(torch.stack([out1, out2, out3, out4]).transpose(0, 1), cur_task.unsqueeze(2)).view(-1, 8)


        #return out, rep, td_e0[:,0,:,:].unsqueeze(1)#, td_e0[:,1,:,:].unsqueeze(1)  # , task_loss_item

        return out, td_e0  # , task_loss_item
